...
<div class="card">
    <div class="card-header header-elements">
        ...
        <div class="card-header-elements ms-auto">
            <a href="{{ route('create.meal.type')}}" type="button" class="btn btn-primary waves-effect waves-light">
...
<script>
    ...
    async function getList() {
        showLoader();
        try {
            let res = await axios.get("/user/get/meal-types");
            if (res.status === 200 && res.data.status === 'success') {
                ...
                res.data.data.forEach(function (item, index) {
                    ...
                    let mealTypeName = item.name ? item.name.charAt(0).toUpperCase() + item.name.slice(1) : '-';
                    ...
                    let row = `
                        <tr>
                            ...
                            <td>${mealTypeName}</td>
                            <td>
                                <a href="/user/edit/meal-type/${item['id']}" class="btn btn-sm btn-outline-success" title="Edit Product"><span class="mdi mdi-pencil-outline"></span>
                                </a>
