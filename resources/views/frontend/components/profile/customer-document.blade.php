...
// Whether the customer already has uploaded documents (so re-upload isn't forced on update)
let hasDoc1 = false;
let hasDoc2 = false;

// ===== Load profile + populate the form =====
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            document.getElementById('docImg1').src = userData['doc_image1']
                ? (isPdfName(userData['doc_image1']) ? PDF_ICON : "/upload/customer-document/small/" + userData['doc_image1'])
                : "/upload/no_image.jpg";
            document.getElementById('docImg2').src = userData['doc_image2']
                ? (isPdfName(userData['doc_image2']) ? PDF_ICON : "/upload/customer-document/small/" + userData['doc_image2'])
                : "/upload/no_image.jpg";

            // Remember which documents already exist (skip the "required" check on update)
            hasDoc1 = !!userData['doc_image1'];
            hasDoc2 = !!userData['doc_image2'];
...
async function onDocumentUpdate(event) {
    ...
    if (firstName.length === 0) {
        ...
        // Only send a document when a new file is chosen — otherwise the backend keeps the existing one
        if (doc_image1) formData.append('doc_image1', doc_image1);
        if (doc_image2) formData.append('doc_image2', doc_image2);

        try {
            showLoader();
            const res = await axios.post("/store/document/info", formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            if (res.data.status === 'success') {
                successToast(res.data.message || 'Document updated successfully'); // global (config.js)
                window.scrollTo({ top: 0, behavior: 'smooth' });
            } else {
                errorToast(res.data.message || 'An unexpected error occurred'); // global (config.js)
            }
        } catch (error) {
