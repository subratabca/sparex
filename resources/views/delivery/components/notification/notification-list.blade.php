...
async function getList() {
    showLoader();
    try {
        let res = await axios.get("/delivery/notification/list/info");
        ...
        function getNotificationLink(notification) {
            if (notification.data) {
                if (notification.data.order_id) {
                    return `/delivery/order/details/${notification.data.order_id}?notification_id=${notification.id}`;
                } else if (notification.data.complain_id) {
                    return `/delivery/complain/details/${notification.data.complain_id}?notification_id=${notification.id}`;
                } else if (notification.data.product_id) {
                    return `/delivery/product/details/${notification.data.product_id}?notification_id=${notification.id}`;
                } else if (notification.data.client_id) {
                    return `/delivery/account/details/${notification.data.client_id}?notification_id=${notification.id}`;
                } else if (notification.data.customer_complain_id) {
                    return `/delivery/customer-complain/details/${notification.data.customer_complain_id}?notification_id=${notification.id}`;
                }
            }
            return '#'; 
        }
