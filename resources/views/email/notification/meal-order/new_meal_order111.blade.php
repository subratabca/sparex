@php
    // Determine customer and order based on notification type
    $customer = null;
    $order = null;
    $items = collect([]);
    $clientOrderDetails = null;
    
    if (isset($clientMealOrder) && $clientMealOrder) {
        $customer = $clientMealOrder->mealOrder->customer;
        $order = $clientMealOrder->mealOrder;
        $items = isset($items) ? $items : collect([]);
        $clientOrderDetails = $clientMealOrder;
    } elseif (isset($mealOrder) && $mealOrder) {
...
@else
<div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 800px; margin: 0 auto; color: #333; line-height: 1.6; text-align: center; padding: 40px 20px;">
    <h2 style="color: #00466a;">New Meal Order Notification</h2>
