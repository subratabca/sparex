    ...
    <div style="margin: 50px auto; width: 80%; max-width: 600px; padding: 30px 0;">
        ...
        <div style="background: #fff8e1; border-left: 4px solid #ffc107; padding: 15px; margin-bottom: 25px; border-radius: 4px;">
            <p style="margin: 0; color: #2e7d32; font-weight: 500;">
                There is a complaint from a customer regarding this product from
                <strong>order #{{ $complaint->orderItem->order->order_number ?? 'N/A' }}</strong>.
            </p>
            <p style="margin: 0; color: #2e7d32; font-weight: 500;">
                Please review the complaint details below and take appropriate action.
            </p>
            <a href="{{ url('admin/complaint/details/' . $complaint->id) }}" 
            ...
            <p style="margin: 10px 0 0 0; color: #856404; font-weight: 500;">
                Current Status: <strong style="text-transform: capitalize;">{{ $complaint->status ?? 'pending' }}</strong>
            </p>
        </div>
            ...
            <div style="display: flex; gap: 20px; margin-bottom: 20px;">
                ...
                <div style="flex: 1;">
                    <h2 style="color: #00466a; margin: 0 0 10px 0; font-size: 1.5em;">
                        {{ $complaint->orderItem->product->name ?? 'Unnamed Product' }}
                    </h2>
                    ...
                    <div style="margin-bottom: 8px; color: #28a745; font-weight: bold;">
                        ${{ number_format($complaint->orderItem->product->price ?? 0, 2) }}
                        × {{ $complaint->orderItem->quantity ?? 1 }}
                    </div>
                    ...
                    <div style="color: #666;">
                        ...
                        {{ $complaint->orderItem->product->client->firstName ?? '' }}
                        {{ $complaint->orderItem->product->client->lastName ?? '' }}
                    </div>

                    <div style="margin-top: 10px; font-weight: bold; text-align: right;">
                        Total:
                        ${{ number_format(($complaint->orderItem->product->price ?? 0) * ($complaint->orderItem->quantity ?? 1), 2) }}
                    </div>
                </div>
            </div>
