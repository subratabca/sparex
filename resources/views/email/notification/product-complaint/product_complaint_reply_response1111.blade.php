...
@php
    ...
    $link = $recipientType === 'admin' 
        ? 'http://127.0.0.1:8000/admin/complaint/details/'.$complaint->id 
        : ($senderRole === 'seller' 
...
@endphp
        ...
        <div style="background: #fff8e1; border-left: 4px solid #ffc107; padding: 15px; margin-bottom: 25px; border-radius: 4px;">
            @if($recipientType === 'admin')
                <p style="margin: 0; color: #2e7d32; font-weight: 500;">
                    The {{ $senderRole }} has responded to the complaint for <strong>Order #{{ $complaint->orderItem->order->order_number ?? 'N/A' }}</strong>.
                </p>
            ...
            @else
                @if($senderRole === 'seller')
                    <p style="margin: 0; color: #2e7d32; font-weight: 500;">
                        The seller has provided a response regarding your complaint for <strong>Order #{{ $complaint->orderItem->order->order_number ?? 'N/A' }}</strong>.
                ...
                @else
                    <p style="margin: 0 0 10px 0; color: #2e7d32; font-weight: 500;">
                       The customer has provided the response regarding their complaint for <strong>Order #{{ $complaint->orderItem->order->order_number ?? 'N/A' }}</strong>. By clicking below link you can see complaint details.
                    </p>
                @endif
        ...
        <div style="padding: 20px; background: #f8f9fa; border-radius: 8px;">
            <div style="display: flex; gap: 20px; margin-bottom: 20px;">
                @if($complaint->orderItem->product->image ?? false)
                ...
                <div style="flex: 1;">
                    <h2 style="color: #00466a; margin: 0 0 10px 0; font-size: 1.5em;">{{ $complaint->orderItem->product->name ?? 'N/A' }}</h2>
                    
                    @if($complaint->orderItem->product->category ?? false)
                    ...
                    @if($complaint->orderItem->variant ?? false)
                    <div style="margin-bottom: 8px; color: #666;">
