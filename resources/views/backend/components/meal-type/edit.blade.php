...
async function mealTypeInfo() {
    ...
    try {
        ...
        let res = await axios.get("/admin/get/meal-type/details/" + id);
...
async function updateMealType() {
  ...
  if (name.length === 0) {
    ...
    let formData = new FormData();
        ...
        try {
          const res = await axios.post('/admin/update/meal-type', formData, config);
          if (res.status === 200 && res.data.status === 'success') {
            successToast(res.data.message || 'Request success');
            window.location.href = '/admin/meal-types';
            resetCreateForm();
          } else {
