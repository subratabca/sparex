  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel3">Write comments<span class="text-danger">*</span></h4>
        <button
          type="button"
          class="btn-close"
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-success" id="complaint-solved-btn">Complaint Solved</button>
        <button type="button" class="btn btn-danger" id="complaint-investigation-btn">Further Investigation</button>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('complaint-solved-btn').addEventListener('click', function(event) {
      event.preventDefault();
      complaintSolved();
    });

    document.getElementById('complaint-investigation-btn').addEventListener('click', function(event) {
      event.preventDefault();
  ...
  async function complaintSolved() {
    ...
    if (textMessage === '') {
      ...
      showLoader();
      try {
        let res = await axios.post("/admin/complaint/solved", formData, config);
        if (res.status === 201) {
  ...
  async function complaintInvestigation() {
    let htmlMessage = quill.root.innerHTML.trim();
    let textMessage = quill.getText().trim();
    let complaint_id = document.getElementById('complainID').value;
    ...
    if (textMessage === '') {
      ...
      showLoader();
      try {
        let res = await axios.post("/admin/complaint/investigation", formData, config);
        if (res.status === 201) {
  ...
  function handleError(error) {
      ...
      if (error.response) {
          ...
          switch (status) {
              case 400:
                  message = serverMessage || 'Complain is not in pending status';
                  break;
              ...
              case 404:
                  message = serverMessage || 'Complain not found!';
                  break;
                  ...
                  if (errors) {
                      for (let key in errors) {
                          if (errors.hasOwnProperty(key)) {
                              ...
                              if (fieldError) {
                                  fieldError.innerText = errors[key][0];
                              }
                          }
                      }
                      return;
