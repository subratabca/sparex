...
<div class="col-md-6 mb-3">
  <div class="card">
    <div class="card-body">
      <h5>Search by client</h5><hr>
      <div class="row">
        <div class="col-md-4">
          <div>
            <label class="form-label">Start Date: <span class="text-danger">*</span></label>
            <input type="date" class="form-control" id="client-start-date" name="client-start-date" />
            <span id="client-start-date-error" class="text-danger"></span>
          </div>
        </div> 
        <div class="col-md-4">
          <div>
            <label class="form-label">End Date: <span class="text-danger">*</span></label>
            <input type="date" class="form-control" id="client-end-date" name="client-end-date" />
            <span id="client-end-date-error" class="text-danger"></span>
          </div>
        </div>  
        <div class="col-md-4">
    ...
    <div class="card-footer">
      <button class="btn btn-outline-primary" onclick="Save('client')">Search</button>
    </div>
  </div>
</div>
...
<script>
  async function loadClients() {
    try {
      let res = await axios.get("/clients");
  ...
  async function Save(criteria) {
    ...
    document.getElementById('client-start-date-error').innerText = '';
    document.getElementById('client-end-date-error').innerText = '';
    ...
    if (criteria === 'single') {
      ...
      let startDate = document.getElementById('client-start-date').value;
      let endDate = document.getElementById('client-end-date').value;
      let clientId = document.getElementById('clientSelect').value;

      if (!startDate) {
        document.getElementById('client-start-date-error').innerText = 'Please select a start date!';
        return;
      }
      if (!endDate) {
        document.getElementById('client-end-date-error').innerText = 'Please select an end date!';
        return;
      }
      if (!clientId) {
        document.getElementById('client-error').innerText = 'Please select a client!';
        return;
      }
      formData.append('client_start_date', startDate);
      formData.append('client_end_date', endDate);
        ...
        res.data.data.forEach(function (item, index) {
          ...
          let row = `
                <tr>
                  ...
                  <td>
                    <a href="/admin/invoice/download/${item.id}" class="btn btn-sm btn-outline-primary" title="Invoice Download"><span class="mdi mdi-arrow-down-circle-outline"></span>
                    </a>
                    <a href="/admin/order/details/${item.id}" class="btn btn-sm btn-outline-primary" title="Order Details"><span class="mdi mdi-eye-circle"></span>
                    </a>
                  </td>
          </tr>`;
