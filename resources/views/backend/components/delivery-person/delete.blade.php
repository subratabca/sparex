...
<script>
     async function itemDelete() {
        showLoader();
        try {
            let delivery_person_id = document.getElementById('deleteID').value;
            $('#delete-modal').modal('hide');
            let res = await axios.post("/admin/delivery-person/delete", {delivery_person_id: delivery_person_id});
            if (res.status === 200) {
