    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body">
                <input type="hidden" id="clientMealOrderID">
...
function getPayableAmount() {
    return document.getElementById('payableAmount').value || '0';
}

function getClientMealOrderID() {
    return document.getElementById('clientMealOrderID').value || '';
}
...
// ===== Modal Shown =====
document.getElementById('paymentModal').addEventListener('shown.bs.modal', async function () {
    ...
    // Init Stripe once
    if (!creditStripe) {
        creditStripe   = Stripe('pk_test_51TYfnnSysbGNh930BuiBj9caN8tzL4tzG6d1xqJDVKtczLLAEE1hapYZwCR13UTAGkRAwCJ5wID74LrgZkCZGdt000Lus0TDjB');
...
document.querySelectorAll('input[name="credit_payment"]').forEach(radio => {
    radio.addEventListener('change', async function () {
        ...
        if (this.value === 'cash') {
            ...
            // Load admin profile for billing form
...
// ===== Country/County/City — use global config.js with custom element IDs =====
// Note: config.js loadCountries/loadCounties/loadCities target #country/#county/#city
// We use dedicated functions here for modal-specific IDs
...
function validateCreditBillingForm(d) {
    ...
    if (!d.country_code) { document.getElementById('credit-country-error').textContent  = 'Please select a country';   isValid = false; }
...
async function processCashPayment() {
    ...
    const res = await axios.post('/admin/store/meal-order/payment/by/cash', {
        client_meal_order_id: getClientMealOrderID(),
        amount:               parseFloat(amount).toFixed(2),
    });

    if (res.data.status === 'success') {
        successToast(res.data.message || 'Payment submitted successfully!');
        bootstrap.Modal.getInstance(document.getElementById('paymentModal')).hide();
        await getList(); // ← refresh table without full page reload
    } else {
...
async function processStripePayment() {
    ...
    const piRes = await axios.post('/admin/meal-order/create-payment-intent', {
        amount:      Math.round(parseFloat(amount) * 100),
        currency:    'usd',
        description: `Client Payment - £${parseFloat(amount).toFixed(2)}`,
        metadata: {
    ...
    if (paymentIntent?.status === 'succeeded') {
        const res = await axios.post('/admin/store/meal-order/payment/by/stripe', {
            client_meal_order_id:  getClientMealOrderID(),
            amount:                parseFloat(amount).toFixed(2),
...
// ===== infoToast — using global Toastify =====
