...
async function updateOrderItems() {
    ...
    orderItemsList.innerHTML = itemsToShow.map(item => {
        ...
        const clientOrder = orderData.client_orders.find(co => co.client_id === item.client_id);
        const clientStatus = clientOrder ? clientOrder.status : 'N/A';

        // Build per-item buttons (🚫 removed Deliver button)
        let actionButtons = '';
        if (item.status === 'pending') {
            actionButtons = `
                <button class="btn btn-sm btn-success me-1" onclick="approveItem(${item.id})">Approve</button>
                <button class="btn btn-sm btn-danger" onclick="openCancelModal(${item.id})">Cancel</button>
            `;
        }
    ...
    // 🔹 Add global delivery buttons (per client)
    if (globalDeliveryContainer) {
        globalDeliveryContainer.innerHTML = '';
        orderData.client_orders.forEach(clientOrder => {
            const clientItems = orderItems.filter(i => i.client_id === clientOrder.client_id);
            if (clientItems.length === 0) return;

            // Check if all items are not pending
            const allProcessed = clientItems.every(i => i.status !== 'pending');
            if (allProcessed) {
                // At least one approved?
                const hasApproved = clientItems.some(i => i.status === 'approved');
                if (hasApproved) {
                    globalDeliveryContainer.innerHTML += `
                        <button class="btn btn-primary me-2 mb-2"
                            onclick="deliverItem(${orderData.order.id})">
                            Deliver Order
                        </button>
                    `;
                }
...
async function deliverItem(orderId) {
    try {
        ...
        if (res.status === 200 && res.data.status === 'success') {
            ...
            errorToast(res.data.message || "Delivery failed");
        }
    } catch (error) {
        handleError(error);
    } finally {
