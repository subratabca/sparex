...
class DeliveryChargeLedger extends Model
{
    ...
    // Get total number of items for this delivery charge
    public function getTotalItemsAttribute()
    {
        return $this->mealOrderItems->count();
    }

    // Get total quantity of items for this delivery charge
    public function getTotalQuantityAttribute()
    {
        return $this->mealOrderItems->sum('quantity');
    }
}
...
Earnings Management: All delivery person earnings are tracked directly in this model through the delivery_charge field and payment_status.

Helper Methods for Earnings:

totalEarningsForDeliveryPerson(): Calculate total earnings for a delivery person

totalDueEarningsForDeliveryPerson(): Calculate due earnings

totalPaidEarningsForDeliveryPerson(): Calculate paid earnings

Payment Management:

markAsPaid(): Mark payment as paid

markAsDue(): Mark payment as due

markAsCancelled(): Mark payment as cancelled

Delivery Person Assignment:

assignDeliveryPerson(): Assign delivery person

unassignDeliveryPerson(): Remove delivery person assignment

hasDeliveryPerson(): Check if assigned

Useful Scopes:

unassigned(): Get records without delivery person

assigned(): Get records with delivery person

due(), paid(), cancelled(): Filter by payment status

Status Attributes:

delivery_status: Calculated from meal deliveries

payment_status_text: Human-readable payment status

payment_status_badge: Badge color for UI

delivery_status_badge: Badge color for delivery status

This model now fully manages delivery person earnings without needing a separate DeliveryPersonEarning model.*/