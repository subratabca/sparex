...
class MealOrderController extends Controller
{
    ...
    //Using DeliveryHelper & data insertion into DeliveryChargeLedger model
    public function storeByStripe(Request $request)
    {
        ...
        try {
            ...
            $overallTax = DeliveryHelper::calculateTax($overallSubtotal, $taxRate);
            ...
            foreach ($mealOrdersByDate as $date => $itemsForDate) {
                ...
                foreach ($itemsForDate as $item) {
                    ...
                    $distance = DeliveryHelper::getDistanceBetweenLocations($clientAddress, $customerShippingAddress);
                    ...
                    // Calculate charge using helper
                    $charge = DeliveryHelper::calculateDeliveryCharge($distance, $deliveryCharge);
                    
                    // Determine distance category
                    $category = DeliveryHelper::getDistanceCategory($distance);
            ...
            foreach ($mealOrders as $item) {
                ...
                $clientTaxes[$clientId] += DeliveryHelper::calculateTax($item['total_price'], $taxRate);
    ...
    public function storeByStripe22(Request $request)
    {
        $request->validate([
            'meal_orders' => 'required|array|min:1',
            'meal_orders.*.meal_date' => 'required|date',
    ...
    //Without Using DeliveryHelper
    public function storeByStripe11(Request $request)
    {
        $request->validate([
            'meal_orders' => 'required|array|min:1',
            'meal_orders.*.meal_date' => 'required|date',
    ...
    //implement data insertion into DeliveryChargeLedger model
    ...
    //data insertion into DeliveryChargeLedger model
    ...
    //without data insertion into DeliveryChargeLedger model
    public function storeByCredit11(Request $request)
    {
        $request->validate([
            'meal_orders' => 'required|array|min:1',
            'meal_orders.*.meal_date' => 'required|date',
