...
class MealOrderController extends Controller
{
    ...
    public function getMealOrderDetails($id)
    {
        try {
            ...
            // Get delivery charge ledgers for this order
            $deliveryChargeLedgers = DeliveryChargeLedger::where('meal_order_id', $id)
                ->with(['mealType', 'deliveryPerson:id,firstName,lastName', 'statusHistories' => function($query) {
                    $query->latest()->take(5);
                }])
                ->get()
                ->groupBy(function($ledger) {
                    return $ledger->delivery_date . '_' . $ledger->meal_type_id . '_' . $ledger->client_id;
                });

            // Get delivery statuses
            $deliveryStatuses = DeliveryChargeLedger::STATUS_LABELS;

            // Group items by meal date and meal type with delivery status and meal time
            $groupedItems = [];
            $dates = [];
            $itemsWithTime = [];
            ...
            foreach ($order->items as $item) {
                ...
                if (!isset($groupedItems[$mealDate][$mealTypeName])) {
                    // Find delivery ledger for this date, meal type, and client
                    $ledgerKey = $mealDate . '_' . $mealTypeId . '_' . $clientId;
                    $deliveryLedger = isset($deliveryChargeLedgers[$ledgerKey]) ? 
                        $deliveryChargeLedgers[$ledgerKey]->first() : null;
                    ...
                    if ($deliveryLedger) {
                        $latestStatus = $deliveryLedger->delivery_status;
                        $orderTracking = $deliveryLedger->order_tracking;
                        
                        if ($deliveryLedger->deliveryPerson) {
                            $deliveryPersonName = $deliveryLedger->deliveryPerson->firstName . ' ' . 
                                                $deliveryLedger->deliveryPerson->lastName;
                        }
                        
                        // Get status history
                        $statusHistories = $deliveryLedger->statusHistories->map(function($history) use ($deliveryStatuses) {
                            return [
                    ...
                    $groupedItems[$mealDate][$mealTypeName] = [
                        ...
                        'delivery_status_label' => $deliveryStatuses[$latestStatus] ?? 'Pending',
                ...
                $groupedItems[$mealDate][$mealTypeName]['items'][] = [
                    ...
                    'client_name' => $item->client->firstName . ' ' . $item->client->lastName,
                    'calories' => ($item->product->nutrient->calories ?? 0) * $item->quantity
                ];

                // Add to group total price
                $groupedItems[$mealDate][$mealTypeName]['total_price'] += floatval($item->total_price ?? 0);

                // Store meal time if available
                if ($item->meal_time && !$groupedItems[$mealDate][$mealTypeName]['meal_time']) {
                    $groupedItems[$mealDate][$mealTypeName]['meal_time'] = $item->meal_time;
                    
                    // Add to itemsWithTime for reference
                    $itemsWithTime[] = [
                        'meal_date' => $mealDate,
    ...
    public function getMealOrderDetails11($id)
    {
        try {
            $order = MealOrder::with([
                'items.mealType', 
