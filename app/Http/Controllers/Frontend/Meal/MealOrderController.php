...
class MealOrderController extends Controller
{
    ...
    public function storeByStripe111(Request $request)
    {
        $request->validate([
            'meal_orders' => 'required|array|min:1',
            'meal_orders.*.meal_date' => 'required|date',
    ...
    public function storeByCash111(Request $request)
    {
        $request->validate([
            'meal_orders' => 'required|array|min:1',
            'meal_orders.*.meal_date' => 'required|date',
    ...
    public function getMealOrderDetails($id)
    {
        try {
            $order = MealOrder::with([
                'items.mealType', 
                'items.product',
                'items.client:id,firstName,lastName,mobile,image',
                'mealShippingAddress.country',
                'mealShippingAddress.county', 
                'mealShippingAddress.city',
                'clientMealOrders.client:id,firstName,lastName,mobile,image'
            ])->find($id);
            ...
            // Get delivery charge ledgers for this order - group by ledger id (which is unique per client, meal type, date)
            $deliveryChargeLedgers = DeliveryChargeLedger::where('meal_order_id', $id)
                ->with([
                    'mealType', 
                    'deliveryPerson:id,firstName,lastName,mobile,image',
                    'client:id,firstName,lastName,mobile,image'
                ])
                ->get()
                ->keyBy('id'); // Key by ledger ID for easy access

            // Get delivery statuses
            $deliveryStatuses = DeliveryChargeLedger::STATUS_LABELS;

            // Group items by delivery charge ledger (client + meal type + date)
            $groupedItems = [];
            $dates = [];
            ...
            foreach ($order->items as $item) {
                ...
                if (!in_array($mealDate, $dates)) {
                    $dates[] = $mealDate;
                }

                // Find the delivery ledger for this combination
                $ledger = $deliveryChargeLedgers->first(function ($ledger) use ($mealDate, $mealTypeId, $clientId) {
                    return $ledger->delivery_date == $mealDate && 
                           $ledger->meal_type_id == $mealTypeId && 
                           $ledger->client_id == $clientId;
                });

                $ledgerId = $ledger ? $ledger->id : 'temp_' . $mealDate . '_' . $mealTypeId . '_' . $clientId;

                if (!isset($groupedItems[$mealDate])) {
                    $groupedItems[$mealDate] = [];
                }

                if (!isset($groupedItems[$mealDate][$ledgerId])) {
                    // Get client info
                    $client = $item->client;
                    $clientName = $client ? $client->firstName . ' ' . $client->lastName : 'Unknown Provider';

                    // Get delivery info from ledger
                    $deliveryStatus = $ledger ? $ledger->delivery_status : 'pending';
                    $deliveryPersonName = $ledger && $ledger->deliveryPerson ? 
                        $ledger->deliveryPerson->firstName . ' ' . $ledger->deliveryPerson->lastName : 'Not Assigned';
                    $orderTracking = $ledger ? $ledger->order_tracking : null;

                    $groupedItems[$mealDate][$ledgerId] = [
                        'ledger_id' => $ledgerId,
                        ...
                        'delivery_status_label' => $deliveryStatuses[$deliveryStatus] ?? 'Pending',
                ...
                // Add item to group
                $groupedItems[$mealDate][$ledgerId]['items'][] = [
                    'id' => $item->id,
                ...
                $groupedItems[$mealDate][$ledgerId]['total_price'] += floatval($item->total_price ?? 0);

                // Store meal time if available
                if ($item->meal_time && !$groupedItems[$mealDate][$ledgerId]['meal_time']) {
                    $groupedItems[$mealDate][$ledgerId]['meal_time'] = $item->meal_time;
                }
            }
