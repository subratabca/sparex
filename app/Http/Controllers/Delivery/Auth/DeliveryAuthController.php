...
use App\Notifications\Delivery\DeliveryRegistrationNotification;
...
class DeliveryAuthController extends Controller
{
    ...
    {
        return view('delivery.pages.terms-condition.registration-terms-condition-page');
    }
    ...
    {
        return view('delivery.pages.auth.registration-page');
    }
    ...
    public function Registration(Request $request)
    {
        try {
            ...
            if ($delivery) {
                ActivityLogger::beforeAuthLog('registration_success', 'New delivery man registered successfully.', $request, 'users');
    ...
    public function VerifyDelivery(Request $request)
    {
        ...
        if ($user) {
            if ($user->role === 'delivery' && $user->is_email_verified == 0) {
                ...
                return view('delivery.pages.auth.login-page')->with('message', 'Your account is activated. You can login now.');
            }
        } else {
            ActivityLogger::beforeAuthLog('email_verified_failed', 'Email already verified.', $request, 'users');
            return view('delivery.pages.auth.login-page')->with('message', 'Delivery man not found.');
        }
    }

    public function LoginPage()
    {
        return view('delivery.pages.auth.login-page');
    }
        ...
        try {
            ...
            if ($user !== null) {
                if ($user->is_email_verified == 0) {
                    ActivityLogger::beforeAuthLog(
                        'login_failed',
                        'Account not activated. Delivery needs to verify email.',
                        $request,
                        'users'
                    );
                    return response()->json([
    ...
    {
        return view('delivery.pages.auth.send-otp-page');
    }
    ...
    {
        return view('delivery.pages.auth.verify-otp-page');
    }
    ...
    {
        return view('delivery.pages.auth.reset-password-page');
    }
